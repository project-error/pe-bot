...
export default class WarnCommand extends Command {
  ...
  async exec(
    ...
    try {
      ...
      await warnsRepo.createNewWarning({
        reason: reason,
        userId: member.id,
        modId: msg.author.id,
      });
      const [dmEmbed, modEmbed] = this._buildEmbeds(msg, member, reason, 'User Warned');
      try {
        member.send(dmEmbed);
      } catch (e) {
      ...
      await msg.channel.send(
        makeSimpleEmbed(
          `**User Warned:** \`${member.user.username}\` \n**Reason:** \`${reason}\``
        )
      );
      await this._sendToModLog(modEmbed);
    } catch (e) {
      await msg.channel.send(makeErrorEmbed(e));
